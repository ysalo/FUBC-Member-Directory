...
My Group shows the signed-in deacon's single group and their fellow deacon's phone number from the linked member record, or Phone: Not provided when absent. Email is not displayed. Link each deacon account to their directory member through account administration; no separate deacon phone field is needed. Members is the default view; Birthdays is a separate selectable view for the next 30 days. The total member count is centered at the bottom, not in the group heading. Deacons retain full-directory access but do not gain editing privileges merely through designation. Labels support English and Ukrainian.
