...
Visitation new/edit/details share the circular member-page back control and consistent headings. The date input and its fieldsets have explicit minimum/maximum width constraints for mobile Safari. The person selector is an inline searchable, scrollable list with selected state, missing-results feedback, and Change control. The visitation list has a GET search field that matches member/pastor/deacon names, meeting location, and localized status.
