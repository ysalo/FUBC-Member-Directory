...
async function uploadPhoto(
  ...
  if (photo.size > 4 * 1024 * 1024)
    throw new Error("Photos must be 4 MB or smaller.");
...
export async function addMember(formData: FormData) {
  ...
  const { error } = await supabase.from("people").insert({
    ...
    membership_joined_at: value(formData, "membershipJoinedAt") || null,
...
export async function updateMember(memberId: string, formData: FormData) {
  ...
  if (error) {
    if (newPhotoPath)
      await supabase.storage.from("member-photos").remove([newPhotoPath]);
    throw new Error("Unable to update this member.");
  }
  if (
    current.photo_path &&
    !current.photo_path.startsWith("/") &&
    current.photo_path !== photoPath
  ) {
