...
export type AccountProfile = {
  ...
  provider: string; role: AppRole; status: AccountStatus; person_id: string | null;
  decision_note: string | null; created_at: string; reviewed_at: string | null;
};

export function accessPath(status: AccountStatus) { return status === "active" ? "/" : `/access/${status}`; }
...
export async function requireActiveProfile() {
  ...
  if (context.profile.status !== "active") redirect(accessPath(context.profile.status));
  return { supabase: context.supabase, profile: context.profile };
}

export async function requireEditor() {
  const context = await requireActiveProfile();
  if (!(["editor", "admin"] as AppRole[]).includes(context.profile.role)) redirect("/");
  return context;
}
