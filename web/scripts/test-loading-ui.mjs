...
for (const language of ["en", "uk"]) {
  locale = language;
  pages[language] = renderToStaticMarkup(await loading.default({}));
}
const server = createServer((req, res) => {
  const url = new URL(req.url, "http://fixture");
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.end(
    `<!doctype html><html lang="${url.searchParams.get("locale") || "en"}" data-theme="${url.searchParams.get("theme") || "light"}"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${css}</style><body>${pages[url.searchParams.get("locale") || "en"]}</body></html>`,
  );
});
...
try {
  ...
  for (const language of ["en", "uk"])
    for (const theme of ["light", "dark"]) {
      await page.goto(
        `http://127.0.0.1:${server.address().port}/?locale=${language}&theme=${theme}`,
      );
      ...
      await page.screenshot({
        path: resolve(output, `${language}-${theme}.png`),
        fullPage: true,
        animations: "disabled",
      });
    }
