...
await writeFile(
  ...
  `import React from 'react';export const useRouter=()=>({push:(url)=>{window.destination=url;},refresh:()=>window.dispatchEvent(new Event('refresh-fixture'))});export default function Link({children,...props}){return <a {...props}>{children}</a>;}export async function mutateVisit(op,form){window.actions.push({op,...Object.fromEntries(form),deacons:form.getAll('deacon')});if(window.failNext){window.failNext=false;return {error:'Unable to save. Your draft has been retained.'};}if(op==='respond'){window.demo.visit_recipients[0].response=form.get('decision');window.demo.visit_recipients[0].decline_reason=form.get('decision')==='declined'?form.get('reason'):'';}if(op==='close')window.demo.status=form.get('decision');return {id:'saved-fixture'};}`,
);
await writeFile(
  resolve(dir, "entry.tsx"),
  `import React,{useState,useEffect} from 'react';import{createRoot}from'react-dom/client';import VisitForm from ${JSON.stringify(resolve(root, "src/components/visit-form.tsx"))};import VisitControls from ${JSON.stringify(resolve(root, "src/components/visit-controls.tsx"))};
window.actions=[];window.demo={id:'visit',pastor_id:'pastor',person_id:'member',member_name:'A very long fictional member name for accessibility testing',member_address:'123 Example Street, Seattle, WA',location:'Church meeting room',scheduled_at:'2099-09-15T19:30:00Z',notes:'Discuss the upcoming visit',status:'open',revision:2,visit_recipients:[{deacon_id:'d1',deacon_name:'First Deacon',response:'accepted',decline_reason:'',last_viewed_revision:1},{deacon_id:'d2',deacon_name:'Second Deacon',response:'pending',decline_reason:'',last_viewed_revision:0}]};
function App(){const[version,setVersion]=useState(0);useEffect(()=>{const update=()=>setVersion(v=>v+1);window.addEventListener('refresh-fixture',update);return()=>window.removeEventListener('refresh-fixture',update);},[]);const p=new URLSearchParams(location.search),mode=p.get('mode'),locale=p.get('locale')||'en';return <main className="mx-auto max-w-xl p-5 bg-[var(--app-surface)] text-[var(--app-ink)]"><h1 className="mb-5 text-2xl font-semibold">{mode==='deacon'?'Visit invitation':mode==='edit'?'Edit request':'Request visit'}</h1>{mode==='deacon'?<><p>{window.demo.member_name}</p><p>{window.demo.location}</p><p data-testid="response">{window.demo.visit_recipients[0].response}</p><VisitControls visit={{...window.demo}} userId="d1" locale={locale} canComplete={false}/></>:<VisitForm member={{id:'member',name:window.demo.member_name,address:window.demo.member_address,groupId:p.get('missing')?null:'group'}} deacons={[{id:'d1',name:'First Deacon',group_id:'group'},{id:'d2',name:'Second Deacon',group_id:'group'},{id:'d3',name:'Other Deacon',group_id:null}]} visit={mode==='edit'?window.demo:undefined} locale={locale} zone="America/Los_Angeles" submission="fixture-submission"/>}</main>};createRoot(document.getElementById('root')).render(<App/>);`,
);
