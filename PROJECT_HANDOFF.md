...
Birthday notification demo: assigned active deacons see an accessible on/off switch above the list in their group's Birthdays view. Preference is stored on this device, separately for each account/group; explicitly labeled demo-only. No browser notification permission, push delivery, service worker, scheduler, database migration, or private member caching is added. Expo native UI skill guidance informed the contextual switch row, touch size, labels, and dark-mode styling; the app remains Next.js, not Expo.
