...
-- Updating unrelated member details must not transiently remove an unchanged
-- Deacon ministry row. The leadership trigger legitimately removes a group
-- assignment when Deacon is actually removed, so reconcile the join table by
-- difference instead of deleting and recreating every ministry membership.
...
begin
  ...
  if p_id is null then
    ...
    values(trim(p_data->>'name'),case when has_ministry_ids then primary_ministry else coalesce(p_data->>'ministry','') end,
