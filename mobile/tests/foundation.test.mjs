...
test('appearance preferences theme browser-only visitation surfaces', async () => {
  ...
  assert.match(appearance, /accent: "#765B3B", accentSoft: "#EFE3D2"/);
  assert.match(appearance, /accent: "#D8C3A5", accentSoft: "#382F27"/);
  assert.match(visitation, /accent: adaptive\("#765B3B", "#D8C3A5", "accent"\)/);
  assert.match(globalCss, /--app-accent: #765b3b;/);
  assert.match(globalCss, /--app-accent: #d8c3a5;/);
  assert.match(layout, /tintColor=\{palette\.accent\}/);
  assert.doesNotMatch(`${appearance}\n${visitation}\n${globalCss}\n${layout}`, /#(?:8A6418|E8C878|F4E8C8|3B321F)/i);
