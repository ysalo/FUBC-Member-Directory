...
import {
    acceptsDateFieldValue,
    localDateValue,
} from "../src/features/forms/date-field.ts";
import {
    birthdayNotificationsSupported,
    disableBirthdayNotifications,
    syncBirthdayNotifications,
} from "../src/features/groups/birthday-notifications.web.ts";
import {
    Alert,
    dismissAllWebAlerts,
...
test("install help detects iPhone, iPad desktop mode, Android, and desktop browsers", () => {
    ...
    assert.equal(
        detectInstallPlatform("Mozilla/5.0 (Linux; Android 15; Pixel 9)"),
        "android",
    );
    assert.equal(
        detectInstallPlatform(
...
test("browser share sends only the requested contact and does not copy after cancellation", async () => {
    ...
    const data = [];
    assert.equal(
        await shareBrowserContact("Jane\n123", "Jane", {
            share: async (value) => {
                data.push(value);
            },
        }),
    ...
    assert.equal(
        await shareBrowserContact("Jane", "Jane", {
            share: async () => {
...
test("web dates honor the local calendar and birthday maximum without timezone conversion", () => {
    ...
    for (const value of [
        ...
        assert.equal(
            acceptsDateFieldValue(value, "date", maximum),
            false,
            value,
        );
    }
...
test("web time accepts midnight and last minute but rejects overflow", () => {
    for (const value of ["00:00", "23:59", "09:30", ""])
        assert.equal(acceptsDateFieldValue(value, "time"), true);
    for (const value of ["24:00", "09:60", "9:30", "12:30:01"])
        assert.equal(acceptsDateFieldValue(value, "time"), false);
});
...
test("visit planning uses avatar rows, responsive scheduling fields, and optional participants", async () => {
    const [form, nativeDateTimeField, repository, copy] = await Promise.all([
        ...
        readFile(
            new URL(
                "../src/features/visitation/SupabaseVisitationRepository.ts",
                import.meta.url,
            ),
            "utf8",
        ),
        readFile(
            new URL("../src/features/visitation/copy.ts", import.meta.url),
            "utf8",
        ),
    ]);
    assert.match(form, /<ProfileAvatar[\s\S]*?name=\{person\.name\}/);
    assert.match(form, /<ProfileAvatar[\s\S]*?name=\{participant\.name\}/);
    assert.match(form, /desktop \? styles\.fieldRow : styles\.fieldStack/);
    assert.match(form, /const choicePageSize = 5/);
    assert.match(form, /filteredPeople\.slice\(0, visiblePeopleCount\)/);
...
test("dialog cleanup removes private content and disables stale actions without confirming or canceling", () => {
        ...
        querySelector(selector) {
            return this.children.find((child) =>
                selector.includes(":not")
                    ? child.dataset.kind !== "destructive"
                    : child.dataset.kind === "cancel",
            );
        }
        focus() {}
        showModal() {
    ...
    try {
        ...
        if (originalElement)
            Object.defineProperty(globalThis, "HTMLElement", originalElement);
