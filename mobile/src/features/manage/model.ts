...
export type ManagedMinistry = {
    id: string;
    name: string;
    nameUk?: string | null;
    systemKey?: "pastor" | "deacon" | null;
    archived: boolean;
    revision?: number;
};
...
export function orderedAccounts(
    ...
    const statusOrder: Record<ManagedAccount["status"], number> = {
        pending: 0,
        active: 1,
        denied: 2,
        revoked: 3,
    ...
    switch (action.type) {
                ...
                members: state.members.map((member) =>
                    member.id === action.memberId
                        ? {
                              ...
                              group: member.archived ? member.group : "",
                              leftAt: member.archived
                                  ? null
        ...
        case "link-account": {
            ...
            return {
                ...
                accounts: state.accounts.map((account) =>
                    account.id === action.accountId
                        ? { ...account, personId: action.personId }
                        : account,
                ),
                members: state.members.map((member) =>
                    member.id === action.personId
        ...
        case "unlink-account":
            return {
                ...
                accounts: state.accounts.map((account) =>
                    account.id === action.accountId
                        ? { ...account, personId: null, status: "pending" }
                        : account,
                ),
            };
        default:
