...
const labels = {
  en: { loading: "Loading groups…", error: "Groups could not be loaded.", retry: "Try again", empty: "No active groups", memberCount: (count: number) => `${count} members`, deaconCount: (count: number) => `${count} of 2 deacons` },
  uk: { loading: "Завантаження груп…", error: "Не вдалося завантажити групи.", retry: "Спробувати ще раз", empty: "Немає активних груп", memberCount: (count: number) => `${count} учасників`, deaconCount: (count: number) => `${count} з 2 дияконів` },
} as const;
...
export function GroupsManagementScreen() {
  ...
  return <FlatList contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content} data={data.groups} keyExtractor={(group) => group.id} ListEmptyComponent={<Text selectable style={[styles.empty, { color: palette.secondaryText }]}>{copy.empty}</Text>} renderItem={({ item }) => <Pressable accessibilityRole="button" onPress={() => router.push(`/manage/group/${item.id}` as Href)} style={({ pressed }) => [styles.row, { backgroundColor: palette.surface, borderColor: palette.line }, pressed && styles.pressed]}><View style={styles.copy}><Text selectable style={[styles.name, { color: palette.text }]}>{item.name}</Text><Text selectable style={[styles.detail, { color: palette.secondaryText }]}>{copy.memberCount(item.memberIds.length)} · {copy.deaconCount(item.deaconIds.length)}</Text></View><Ionicons color={palette.secondaryText} name="chevron-forward" size={20} /></Pressable>} style={{ backgroundColor: palette.background }} />;
}

const styles = StyleSheet.create({ content: { gap: 10, padding: 18, paddingBottom: 48 }, row: { alignItems: "center", borderCurve: "continuous", borderRadius: 16, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", minHeight: 70, padding: 16 }, copy: { flex: 1, gap: 4 }, name: { fontSize: 18, fontWeight: "700" }, detail: { fontSize: 14 }, pressed: { opacity: 0.72 }, empty: { paddingTop: 40, textAlign: "center" }, state: { alignItems: "center", flex: 1, gap: 10, justifyContent: "center", padding: 28 }, stateTitle: { fontSize: 18, fontWeight: "700", textAlign: "center" }, retry: { borderCurve: "continuous", borderRadius: 12, paddingHorizontal: 18, paddingVertical: 12 }, retryText: { color: "#FFF", fontWeight: "700" } });
