...
export function MemberProfileScreen({ memberId }: { memberId: string }) {
    ...
    if (profile === undefined)
        return <ProfileState loading title={copy.loading} />;
    if (!profile || failed)
        return (
            <ProfileState
                action={failed ? load : undefined}
                detail={failed ? undefined : copy.unavailableDetail}
                title={failed ? copy.error : copy.unavailable}
            />
        );
    ...
    const responsibilityGroup =
        locale === "uk"
            ? profile.responsibilityGroupUk
            : profile.responsibilityGroup;
    const ministries =
        locale === "uk" ? profile.ministriesUk : profile.ministries;
    const age = profile.birthDate
        ? Math.floor(
              (Date.now() -
                  new Date(`${profile.birthDate}T12:00:00`).getTime()) /
                  31557600000,
          )
        : null;
    ...
    const contactSection = (
        <Section title={copy.contact}>
            <View style={styles.actions}>
                ...
                {profile.address ? (
                    <Action
                        ...
                        label={copy.maps}
                        onPress={() => {
                            const urls = mapUrls(profile.address!);
                            void open(
                                urls.primary,
                                copy.mapsUnavailable,
                                urls.fallback,
                            );
                        }}
                    />
            ...
            {profile.phone ? (
                <ContactValue
                    accessibilityLabel={copy.copyPhone}
                    copiedLabel={copy.copied}
                    emphasized
                    failureDetail={copy.copyUnavailable}
                    failureTitle={copy.actionUnavailable}
            ...
            {profile.address ? (
                <ContactValue
                    accessibilityLabel={copy.copyAddress}
                    copiedLabel={copy.copied}
                    failureDetail={copy.copyUnavailable}
                    failureTitle={copy.actionUnavailable}
    ...
    const detailsSection = (
        <Section title={copy.details}>
            ...
            {age !== null ? (
                <Fact label={copy.age} value={String(age)} />
            ) : null}
            {profile.membershipJoinedAt ? (
                <Fact
                    label={copy.memberSince}
                    value={formatDate(profile.membershipJoinedAt)}
                />
            ) : null}
            ...
            {profile.membershipGroupId ? (
                <Link href={`/groups/${profile.membershipGroupId}`} asChild>
                    <ProfileGroupLinkContent
                        accessibilityHint={copy.openGroup}
                        accessibilityLabel={`${copy.group}: ${group || "—"}`}
                        accessibilityRole="link"
                    >
                        <Fact
                            disclosure
                            label={copy.group}
                            last={!profile.responsibilityGroupId}
                            value={group || "—"}
                        />
                    </ProfileGroupLinkContent>
                </Link>
            ...
            {profile.responsibilityGroupId ? (
                <Link href={`/groups/${profile.responsibilityGroupId}`} asChild>
                    <ProfileGroupLinkContent
                        accessibilityHint={copy.openGroup}
                        accessibilityLabel={`${copy.responsibleFor}: ${responsibilityGroup || "—"}`}
                        accessibilityRole="link"
                    >
                        <Fact
                            disclosure
                            label={copy.responsibleFor}
                            last
        ...
        <Section title={copy.ministries}>
            <View style={styles.bodyList}>
                {profile.leadershipMinistry ? (
                    <LeadershipBadge
                        leadershipMinistry={profile.leadershipMinistry}
                        locale={locale}
                    />
                ) : null}
                {ministries.map((ministry, index) => (
                    <Text
                        key={`${profile.id}:detail-ministry:${index}`}
                        style={[styles.bodyListText, { color: palette.text }]}
                    >
                        {ministry}
                    </Text>
                ))}
        ...
        <View style={[styles.root, { backgroundColor: palette.background }]}>
            <ScrollView
                ...
                contentContainerStyle={[
                    styles.screen,
                    Platform.OS === "web" && styles.webScreen,
                        ...
                        <View style={styles.desktopIdentity}>
                            ...
                            <View style={styles.desktopIdentityCopy}>
                                ...
                                <View style={styles.ministryRow}>
                                    {ministries.map((ministry, index) => (
                                        <Text
                                            key={`${profile.id}:desktop-ministry:${index}`}
                                            style={[
                                                styles.desktopMinistryText,
                                                {
                    ...
                    <View style={[styles.hero, { paddingTop: insets.top }]}>
                        ...
                        {canEdit ? (
                            <Pressable
                                ...
                                onPress={() =>
                                    router.push(
                                        `/manage/member/${memberId}` as Href,
                                    )
                                }
                                style={[
                                    styles.floatingButton,
                        ...
                        <View style={styles.heroCopy}>
                            ...
                            <View style={styles.ministryRow}>
                                {ministries.map((ministry, index) => (
                                    <Text
                                        key={`${profile.id}:hero-ministry:${index}`}
...
function Action({
    ...
    icon: "call-outline" | "mail-outline" | "map-outline" | "share-outline";
    label: string;
    onPress: () => void;
}) {
    const { palette } = useAppearance();
    return (
        <Pressable
            accessibilityRole="button"
            onPress={onPress}
            style={[styles.action, { backgroundColor: palette.accentSoft }]}
        >
            <Ionicons color={palette.accent} name={icon} size={23} />
            <Text
                numberOfLines={1}
                style={[styles.actionText, { color: palette.accent }]}
            >
                {label}
            </Text>
        </Pressable>
...
function ContactValue({
    ...
    const copyValue = async () => {
        try {
            ...
            Alert.alert(failureTitle, failureDetail);
        }
    };
    return (
        <View style={[styles.contactValue, { borderTopColor: palette.line }]}>
            <Text
                selectable
                style={[
                    styles.contactText,
                    emphasized && styles.contactTextEmphasized,
                    {
                        color: emphasized
                            ? palette.text
            ...
            <Pressable
                accessibilityLabel={copied ? copiedLabel : accessibilityLabel}
                accessibilityRole="button"
            ...
            >
                <Ionicons
                    accessibilityElementsHidden
                    color={copied ? palette.accent : palette.secondaryText}
...
function Fact({
    ...
    return (
        ...
        >
            <Text style={[styles.factLabel, { color: palette.secondaryText }]}>
                {label}
            </Text>
            <View style={styles.factValueRow}>
                <Text
                    ...
                    style={[
                        styles.factValue,
                        { color: disclosure ? palette.accent : palette.text },
                    ]}
                >
                    {value}
                </Text>
...
const styles = StyleSheet.create({
    ...
    heroShade: {
        backgroundColor: "rgba(5,10,14,0.25)",
        bottom: 0,
        left: 0,
        position: "absolute",
        right: 0,
    ...
    editButton: { left: "auto", right: 16 },
    heroCopy: { bottom: 24, left: 20, position: "absolute", right: 20 },
    name: {
        color: "#FFF",
        fontSize: 38,
        fontWeight: "800",
    ...
    heroPill: {
        backgroundColor: "rgba(13,18,22,0.7)",
        borderRadius: 8,
        paddingHorizontal: 9,
        paddingVertical: 4,
    },
    ...
    actions: { flexDirection: "row", flexWrap: "wrap", gap: 9, padding: 12 },
    action: {
        alignItems: "center",
        borderRadius: 12,
        flex: 1,
    ...
    contactText: { flex: 1, fontSize: 14, lineHeight: 20 },
    contactTextEmphasized: { fontSize: 16, fontWeight: "700" },
    copyButton: {
        alignItems: "center",
        height: 32,
        justifyContent: "center",
    ...
    bodyList: { gap: 8, padding: 14 },
    bodyListText: { fontSize: 15, fontWeight: "600" },
