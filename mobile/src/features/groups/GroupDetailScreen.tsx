...
export function GroupDetailScreen({ groupId }: { groupId: string }) {
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[styles.screen, { backgroundColor: palette.background }]} keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled" stickyHeaderIndices={[1]} style={{ backgroundColor: palette.background }}>
    <View style={styles.topContent}>
      ...
      <Section title={copy.responsibleDeacons}>{deacons.length ? deacons.map((member) => <PersonRow key={member.id} leader member={member} detail={locale === "uk" ? "Диякон" : "Deacon"} onPress={() => router.push(`/members/${member.id}` as never)} />) : <Text selectable style={[styles.empty, { color: palette.secondaryText }]}>{copy.noDeacons}</Text>}</Section>
      {assignedDeacon ? <Section title={copy.birthdays}>
        ...
        {upcoming.length ? upcoming.map((member) => <PersonRow key={member.id} member={member} detail={formatBirthday(member.month, member.day, locale)} onPress={() => router.push(`/members/${member.id}` as never)} />) : <Text selectable style={[styles.empty, { color: palette.secondaryText }]}>{copy.noUpcomingBirthdays}</Text>}
      </Section> : null}
    </View>
    ...
    <Section title={`${copy.members} (${filteredMembers.length})`}>{filteredMembers.length ? filteredMembers.map((member) => <PersonRow key={member.id} member={member} detail={member.designation === "deacon" ? (locale === "uk" ? "Диякон" : "Deacon") : undefined} onPress={() => router.push(`/members/${member.id}` as never)} />) : <Text selectable style={[styles.empty, { color: palette.secondaryText }]}>{query.trim() ? copy.noMatchingMembers : copy.noMembers}</Text>}</Section>
