...
import { ActionButton, ScreenState, SectionCard, visitColors } from "./VisitationUi";

type FormLoadState =
  | { status: "loading" }
  | { status: "error"; message: string }
  | { status: "ready"; snapshot: RepositorySnapshot; visit: VisitRecord | null };
...
export function VisitFormScreen({ visitId = null, initialPersonId = "" }: { visitId?: string | null; initialPersonId?: string }) {
  ...
  const [mutation, setMutation] = useState<{ status: "idle" | "pending" | "error" | "conflict"; message?: string }>({ status: "idle" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({ status: "error", message: error instanceof VisitRepositoryError && error.code === "forbidden" ? c.forbidden : c.notFoundDetail });
    }
  ...
  const filteredPeople = useMemo(() => {
    ...
    return state.snapshot.people.filter((person) => !needle || person.name.toLocaleLowerCase(locale).includes(needle));
  }, [locale, query, state]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(values.date)) throw new Error(c.validationDate);
    ...
    try {
      const visit = editing && state.visit
        ? await visitationRepository.update(state.visit.id, { scheduledAt, location: values.location, notes: values.notes }, state.visit.revision)
      ...
      setMutation({
        status: error instanceof VisitRepositoryError && error.code === "conflict" ? "conflict" : "error",
        message: error instanceof VisitRepositoryError && error.code === "forbidden" ? c.forbidden : error instanceof Error ? error.message : c.saveFailed,
      });
    }
  };
...
<SectionCard title={c.person}>
          {editing && selectedPerson ? (
            <View style={styles.readonlyRow}>
              ...
              <Text selectable style={styles.readonlyValue}>{selectedPerson.name}</Text>
            </View>
          ) : (
            <>
              <View style={styles.searchField}>
                <Ionicons accessibilityElementsHidden color={visitColors.secondaryText} name="search-outline" size={20} />
                <TextInput
                  ...
                  return (
                    ...
                    >
                      <View style={styles.flex}>
                        <Text selectable style={styles.choiceTitle}>{person.name}</Text>
                        <Text numberOfLines={2} selectable style={styles.choiceDetail}>{person.address}</Text>
                      </View>
                      {selected ? <Ionicons accessibilityElementsHidden color={visitColors.accent} name="checkmark-circle" size={23} /> : null}
                    </Pressable>
                  );
                })}
                {filteredPeople.length === 0 ? <Text selectable style={styles.emptyText}>{c.noResults}</Text> : null}
              </View>
            </>
          )}
        </SectionCard>
...
<SectionCard>
          ...
          <TextInput
            ...
            maxLength={1000}
            onChangeText={(location) => setValues((current) => ({ ...current, location }))}
            placeholder={c.location}
            placeholderTextColor={visitColors.secondaryText}
            style={styles.input}
            value={values.location}
          />
          <Text style={styles.fieldLabel}>{c.notes} ({c.optional})</Text>
          <TextInput
    ...
    return (
      <>
        ...
        <ActionButton disabled={mutation.status === "pending"} icon="checkmark" label={mutation.status === "pending" ? c.saving : editing ? c.save : c.create} onPress={() => void save()} />
      </>
    );
  })();
...
const styles = StyleSheet.create({
  ...
  readonlyValue: { color: visitColors.text, fontSize: 16, fontWeight: "600", lineHeight: 23 },
  searchField: { alignItems: "center", backgroundColor: visitColors.background, borderColor: visitColors.line, borderCurve: "continuous", borderRadius: 12, borderWidth: 1, flexDirection: "row", gap: 8, minHeight: 46, paddingHorizontal: 12 },
  searchInput: { color: visitColors.text, flex: 1, fontSize: 16, paddingVertical: 10 },
  choiceList: { gap: 8 },
  choiceRow: { alignItems: "center", backgroundColor: visitColors.background, borderColor: visitColors.line, borderCurve: "continuous", borderRadius: 13, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", gap: 10, minHeight: 54, paddingHorizontal: 12, paddingVertical: 10 },
  choiceRowSelected: { backgroundColor: visitColors.accentSoft, borderColor: visitColors.accent },
  choiceTitle: { color: visitColors.text, fontSize: 16, fontWeight: "600", lineHeight: 22 },
  choiceDetail: { color: visitColors.secondaryText, fontSize: 13, lineHeight: 18, paddingTop: 2 },
  groupMatch: { color: visitColors.accent, fontSize: 12, fontWeight: "700", paddingTop: 2 },
  fieldRow: { flexDirection: "row", gap: 10 },
  timeField: { width: 120 },
  fieldLabel: { color: visitColors.text, fontSize: 14, fontWeight: "600", paddingBottom: 6 },
  input: { backgroundColor: visitColors.background, borderColor: visitColors.line, borderCurve: "continuous", borderRadius: 12, borderWidth: 1, color: visitColors.text, fontSize: 16, minHeight: 48, paddingHorizontal: 12, paddingVertical: 11 },
  textArea: { backgroundColor: visitColors.background, borderColor: visitColors.line, borderCurve: "continuous", borderRadius: 12, borderWidth: 1, color: visitColors.text, fontSize: 16, minHeight: 110, padding: 12, textAlignVertical: "top" },
  emptyText: { color: visitColors.secondaryText, fontSize: 14, padding: 12, textAlign: "center" },
