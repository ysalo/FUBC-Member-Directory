...
import { createPrivacySafeNotification, isPrivacySafeNotification } from "./notifications";
...
async function rejectsCode(task: () => Promise<unknown>, code: VisitRepositoryError["code"]) {
  try {
    await task();
  } catch (error) {
    assert(error instanceof VisitRepositoryError, "Expected VisitRepositoryError");
...
const tests: Array<{ name: string; run: () => void | Promise<void> }> = [
  {
    ...
    run: async () => {
      ...
      assert(url.includes("action=TEMPLATE"), "Calendar URL should open a prefilled event");
      assert(url.includes("20270116T013000Z%2F20270116T023000Z"), "Calendar snapshot should contain fixed instant and one-hour end");
      equal(parameters.get("location"), visit.location, "Calendar snapshot should contain the confirmed location");
    },
  },
  {
    name: "fixed PDT stays UTC−07 in summer and winter",
    run: () => {
      equal(fixedPdtToIso("2027-01-15", "18:30"), "2027-01-16T01:30:00.000Z", "Winter conversion");
      equal(fixedPdtToIso("2027-07-15", "18:30"), "2027-07-16T01:30:00.000Z", "Summer conversion");
  ...
  },
  {
    ...
    run: async () => {
      ...
      const duplicate = await repository.create({ ...draft, location: "Different retry payload" });
      equal(duplicate.id, created.id, "Duplicate submission id");
      equal(duplicate.location, created.location, "First submission remains authoritative");
  ...
  },
  {
    ...
    run: async () => {
      ...
      const markoAccepted = await repository.respond("visit-marta", { response: "accepted", reason: null }, initial.revision);
      equal(markoAccepted.recipients.find((recipient) => recipient.accountId === "deacon-marko")?.response, "accepted", "Marko response");
      equal(markoAccepted.recipients.find((recipient) => recipient.accountId === "deacon-leah")?.response, "accepted", "Leah response remains independent");

      const markoDeclined = await repository.respond("visit-marta", { response: "declined", reason: "Schedule conflict" }, markoAccepted.revision);
      equal(markoDeclined.recipients.find((recipient) => recipient.accountId === "deacon-marko")?.reason, "Schedule conflict", "Optional reason stored");
      await repository.setActor("deacon-leah");
      await rejectsCode(() => repository.getAuthorized("visit-daniel"), "not-found");
    },
  },
  {
    ...
    run: async () => {
      ...
      await rejectsCode(() => repository.archive(open.id, open.revision), "invalid");
      ...
      const archived = await repository.archive(completed.id, completed.revision);
      equal(archived.status, "completed", "Archive preserves completion state");
      assert(Boolean(archived.archivedAt), "Archive records an archive timestamp");
    },
  },
  {
    name: "external notification copy excludes visit details",
    run: () => {
      const notification = createPrivacySafeNotification("event-1", "deacon-marko", "visit-marta", "en");
      assert(
        isPrivacySafeNotification(notification, ["Marta Kovalenko", "2480 Fair Oaks Boulevard", "front entrance", "2027-01-15"]),
        "Notification must not contain names, location, notes, or schedule",
      );
      equal(notification.data.visitId, "visit-marta", "Notification deep-link data");
    },
  },
];
