...
export class InMemoryVisitationRepository implements VisitationRepository {
    ...
    private readonly eligibleParticipants = visitationParticipants.map(
    ...
    bindSessionActor(actor: VisitActor | null) {
        ...
        if (
            this.sessionActor &&
            actor &&
            this.sessionActor.id === actor.id &&
            this.sessionActor.revision === actor.revision
        )
            return;
    ...
    async getSnapshot(): Promise<RepositorySnapshot> {
        ...
        if (this.sessionActor !== undefined) {
            if (!this.sessionActor)
                throw new VisitRepositoryError(
                    "forbidden",
                    "No authorized visitation account.",
                );
            return {
                actor: cloneActor(this.sessionActor),
                actors: [cloneActor(this.sessionActor)],
                people: [],
        ...
        return {
            ...
            eligibleParticipants: this.eligibleParticipants.map(
                (participant) => ({ ...participant }),
            ),
        };
    }
    ...
    async setActor(accountId: string) {
        ...
        if (!actor)
            throw new VisitRepositoryError("not-found", "Account not found.");
    ...
    async create(draft: VisitDraft) {
        ...
        this.assertDemoMode();
        const actor = this.actor();
        if (!canCreateVisit(actor))
            throw new VisitRepositoryError(
                "forbidden",
                "Only an active pastor or deacon can plan visits.",
            );

        const duplicate = this.visits.find(
            (visit) =>
                visit.plannerId === actor.id &&
                visit.submissionId === draft.submissionId,
        );
        if (duplicate) return cloneVisit(duplicate);

        const person = this.people.find(
            (candidate) => candidate.id === draft.personId,
        );
        if (!person)
    ...
    async update(id: string, edit: VisitEdit, expectedRevision: number) {
        ...
        if (edit.scheduledAt !== visit.scheduledAt)
            updatedFields.push("scheduledAt");
        if (edit.location.trim() !== visit.location)
    ...
    async respond(
        ...
        const actor = this.actor();
        if (!canRespondToVisit(actor, visit))
            throw new VisitRepositoryError(
                "forbidden",
                "This invitation is not open for your response.",
            );
        const recipient = visit.recipients.find(
            (candidate) => candidate.accountId === actor.id,
        );
        if (!recipient)
            throw new VisitRepositoryError(
                "forbidden",
                "This invitation is not assigned to you.",
            );
        recipient.response = input.response;
        recipient.reason =
            input.response === "declined" ? input.reason?.trim() || null : null;
        visit.revision += 1;
    ...
    async archive(id: string, expectedRevision: number) {
        ...
        const visit = this.find(id);
        this.assertRevision(visit, expectedRevision);
        if (!canManageVisit(this.actor(), visit))
            throw new VisitRepositoryError(
                "forbidden",
                "Only the planner can archive this visit.",
            );
        if (visit.status === "open")
            throw new VisitRepositoryError(
                "invalid",
                "Complete or cancel the visit before archiving it.",
            );
        if (visit.archivedAt)
            throw new VisitRepositoryError(
                "terminal",
                "This visit is already archived.",
            );
    ...
    async markViewed(id: string, revision: number) {
        ...
        if (!canReadVisit(this.actor(), visit))
            throw new VisitRepositoryError("not-found", "Visit unavailable.");
        const recipient = visit.recipients.find(
            (candidate) => candidate.accountId === this.actorId,
        );
        if (recipient)
            recipient.lastViewedRevision = Math.max(
                recipient.lastViewedRevision,
                revision,
            );
    }
    ...
    private actor() {
        ...
        const actor = this.actors.find(
            (candidate) => candidate.id === this.actorId,
        );
        if (!actor)
            throw new VisitRepositoryError("forbidden", "No active account.");
        return actor;
    }
    ...
    private assertDemoMode() {
        if (this.sessionActor !== undefined) {
            throw new VisitRepositoryError(
                "forbidden",
                "The remote visitation adapter must handle configured sessions.",
            );
        }
    }
    ...
    private assertRevision(visit: VisitRecord, expectedRevision: number) {
        if (visit.revision !== expectedRevision) {
            throw new VisitRepositoryError(
                "conflict",
                "The visit changed on another device. Refresh and try again.",
            );
        }
    }
    ...
    private assertManageable(visit: VisitRecord, expectedRevision: number) {
        ...
        if (!canManageVisit(this.actor(), visit))
            throw new VisitRepositoryError(
                "forbidden",
                "Only the planner can change this visit.",
            );
        if (visit.status !== "open" || visit.archivedAt)
            throw new VisitRepositoryError(
                "terminal",
    ...
    private validateOpenFields(scheduledAt: string, location: string) {
        ...
        if (location.length > 1000)
            throw new VisitRepositoryError(
                "invalid",
                "The visit location is too long.",
            );
    }
