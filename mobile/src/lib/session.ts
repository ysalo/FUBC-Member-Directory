...
const revalidation = createSessionRevalidator<Account>({
  ...
  error: (cause) => publish({ status: "error", account: null, error: errorMessage(cause) }),
  async load() {
    ...
    return { id: account.id, personId: account.person_id, displayName: account.display_name, status: account.status, role: account.role, leadershipMinistry: account.leadership_ministry, revision: account.revision };
  },
});
...
export function startSession(): () => void {
  ...
  void supabase.auth.getSession().then(({ data, error }) => {
    ...
    if (error) { revalidation.invalidate(); publish({ status: "error", account: null, error: error.message }); }
    else void resolveAccount(data.session);
  });
  return () => { mounted = false; revalidation.invalidate(); subscription.unsubscribe(); };
}
